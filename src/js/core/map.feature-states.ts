...
import { TerrainStyleDef } from "./styles";
 ...
 */
function createFeatureStateCase(state: string, valueA: any, valueB: any) {
  return [
    "case",
...
export class Map extends maplibreMap {
  ...
  private _setStateFeature(state: string, feature: maplibreMapGeoJSONFeature | undefined, event?: MapFeatureMouseEvent | MapFeatureTouchEvent) {
    ...
    if (this._statesActive.feature[state] && !featureIsSame) {
      if (this._statesActive.feature[state].id !== undefined) {
        this.setFeatureState(this._statesActive.feature[state], { [state]: false });
      }
      if (event) {
    ...
    if (feature && !isLayerHidden(feature.layer) && !featureIsSame) {
      if (feature.id !== undefined) {
        this.setFeatureState(feature, { [state]: true });
      } else {
        console.warn(`${feature.layer.id}: Feature state '${state}' couldn't be set, source feature must provide an id.`);
      }
        ...
        if (style.layers[i].id in this._states.feature[state]) {
          for (const prop of StatePropertyList) {
            for (const lapa in this._states.feature[state][style.layers[i].id][prop]) {
              if (!maplibreSupportsPropertyExpression(maplibreV8[`${prop}_${style.layers[i].type}`][lapa])) {
                console.error(`layers[${i}].${style.layers[i].type}.${prop}.${lapa} doesn't support data expressions and can't use feature states.`);
                continue;
              } else if (maplibreV8[`${prop}_${style.layers[i].type}`][lapa].expression.parameters.includes("feature-state")) {
              ...
              style.layers[i][prop][lapa] = createFeatureStateCase(state, stateValue, currentValue === undefined ? defaultValue : currentValue);
            }
          }
        }
      }
   ...
   * @param update - Whether to update the style immediately.
   * @returns The map instance.
   */
   ...
   * @returns The map instance.
   */
  addLayer(layer: maplibreAddLayerObject | Array<maplibreAddLayerObject>, beforeId?: string) {
